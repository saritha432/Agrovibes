...
import { dismissIncomingCallRinging, parseCallCancelledRemoteMessage } from "./incomingCallSignal";
...
export async function handleFcmRemoteMessage(remoteMessage: FcmRemoteMessage | null | undefined) {
  const cancelled = parseCallCancelledRemoteMessage(remoteMessage);
  if (cancelled) {
    await dismissIncomingCallRinging(cancelled);
    return;
  }
