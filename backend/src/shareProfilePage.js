const { buildProfileDeepLinkUrls, getEstimatedAppSizeLabel } = require("./appDeepLinkUrls");
const { getWebAppOrigin, getApiOrigin, absolutizeMediaUrl, isLinkPreviewBot } = require("./shareReelPage");
...
function profileShareImage(profile) {
  const avatar = stripEnv(profile?.avatarUrl);
  return avatar ? absolutizeMediaUrl(avatar) : "";
}

function profileSnippet(profile, maxLen = 120) {
  const bio = stripEnv(profile?.bio);
  if (bio) {
    if (bio.length <= maxLen) return bio;
    return `${bio.slice(0, Math.max(0, maxLen - 3)).trim()}...`;
  }
  const name = stripEnv(profile?.fullName) || "Cropvibe";
  return `View ${name}'s profile on Cropvibe`;
}

/** Same open-app / Install Cropvibe script as reel shares (urls shape compatible). */
    ...
    <script>
      (function () {
        ...
        function wireInstall() {
          ...
          if (detailsBtn) {
            detailsBtn.href = watchUrl + (watchUrl.indexOf("?") >= 0 ? "&" : "?") + "web=1";
            detailsBtn.addEventListener("click", function (e) {
              e.preventDefault();
              window.location.href = watchUrl + (watchUrl.indexOf("?") >= 0 ? "&" : "?") + "web=1";
            });
          }
        }
...
function buildShareProfileHtml(profile, { userKey, userAgent }) {
  const webOrigin = getWebAppOrigin();
  const key = String(userKey || profile?.id || "").trim();
  const urls = buildProfileDeepLinkUrls(key, webOrigin);
  const authorName = String(profile?.fullName || "Cropvibe").trim() || "Cropvibe";
  const author = escapeHtml(authorName);
  const snippet = profileSnippet(profile);
  const caption = escapeHtml(snippet);
  const title = escapeHtml(`${authorName} on Cropvibe`);
  const image = profileShareImage(profile);
  const canonicalPath = urls.httpsProfileUrl;
  ...
  const handle = stripEnv(profile?.username);
  const handleLine = handle ? `@${escapeHtml(handle)}` : "";
...
<body>
  <div class="page">
    ${
      ...
      <p><strong>${author}</strong>${handleLine ? `<br/>${handleLine}` : ""}</p>
      <p>${caption}</p>
    </div>`
        : `<div class="topbar">
